 ...
 * @return  void
 ***************************************************************************************************/
void MT_ProcessDebugMsg4( uint16 param1, uint16 param2, uint16 param3, uint16 param4 )
{
  uint8 buf[8];
  uint8 *pBuf;
  ...
  *pBuf++ = HI_UINT16( param4 );
  *pBuf++ = LO_UINT16( param4 );

#ifdef MT_UART_DEFAULT_PORT
  /* Debug message is set to AREQ CMD 0x80 for now */
  /* Build and send back the response */
  MT_BuildAndSendZToolResponse(((uint8)MT_RPC_CMD_AREQ | (uint8)MT_RPC_SYS_DBG), 0x80, 8, buf);
#endif
}
 ...
 * @brief   Build and send a debug string.
 *
 * @param   char *dstr - pointer to the data portion of the debug message
 *
 * @return  void
 ***************************************************************************************************/
void MT_ProcessDebugString(const char *str)
{
  ...
  if ( (len > 0) && (len < 255))
  {
    ...
    /* Debug message is set to AREQ CMD 0x80 for now */
    /* Build and send back the response */
    MT_BuildAndSendZToolResponse(((uint8)MT_RPC_CMD_AREQ | (uint8)MT_RPC_SYS_DBG), MT_DEBUG_MSG, len, (uint8*)str);
#endif
  }
}
