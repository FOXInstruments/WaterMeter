...
ZStatus_t zcl_SendRead( uint8 srcEP, afAddrType_t *dstAddr,
                        uint16 clusterID, const zclReadCmd_t *readCmd,
...
ZStatus_t zcl_SendReportCmd( uint8 srcEP, afAddrType_t *dstAddr,
                             uint16 clusterID, const zclReportCmd_t *reportCmd,
                             uint8 direction, uint8 disableDefaultRsp, uint8 seqNum )
{
  uint16 dataLen = 0;
  uint8 *buf;
  ...
  for ( i = 0; i < reportCmd->numAttr; i++ )
  {
    ...
    dataLen += zclGetAttrDataLength( reportCmd->attrList[i].dataType, reportCmd->attrList[i].attrData );
  }
  ...
  if ( buf != NULL )
  {
    ...
    for ( i = 0; i < reportCmd->numAttr; i++ )
    {
      ...
      *pBuf++ = LO_UINT16( reportCmd->attrList[i].attrID );
      *pBuf++ = HI_UINT16( reportCmd->attrList[i].attrID );
      *pBuf++ = reportCmd->attrList[i].dataType;

      pBuf = zclSerializeData( reportCmd->attrList[i].dataType, reportCmd->attrList[i].attrData, pBuf );
